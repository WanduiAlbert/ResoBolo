...
plotdir = 'TKIDModule_higherorder/'
...
if __name__=="__main__":
    ...
    for ires,fn in enumerate(fns):
        ...
        f = Sparams['frequency']
        S21 = np.abs(Sparams['S21'])
        S11 = np.abs(Sparams['S11'])
        funcS21 = interpolate.interp1d(f, S21, kind='cubic')
        funcS11 = interpolate.interp1d(f, S11, kind='cubic')
        Pdiss = 1 - S11**2 - S21**2

        finterp = np.r_[f[0]:f[-1]:5000j]
        S21interp = funcS21(finterp)
        S11interp = funcS11(finterp)
        Pdissinterp = 1 - S11interp**2 - S21interp**2
        ...
        plt.ylabel('Pdiss')
        plt.title('Power dissipated by the resonator with %d IDC sections'%(section))
        lgd = plt.legend(loc="upper left")
        path = os.path.join(plotdir,'reso_%d'%section + "_Pdiss.png")
        plt.savefig(path)
        plt.savefig(path)
        plt.show()
