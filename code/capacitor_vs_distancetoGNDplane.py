...
def admittance_model(x, C, L, R):
	w = 2*pi*x
	w0 = 1./np.sqrt(L*C)
	return np.log(np.abs(1./(R + (2 - (w/w0)**2)/(1j*w*C))))
	return np.log(np.abs(1./(R + 1j*w*L + 1./(1j*w*C))))
...
def get_cap_params(fn):
	...
	nY21 = nY21[f/MHz < 800]
	f = f[f/MHz < 800]
	#print(np.mean(np.gradient(nY21.imag)))
	#C_est = np.mean(np.gradient(nY21.imag))/np.mean(2*pi*f)
	#L_est = 0.01*nH
	#R_est = 1e-5
	...
	y_fit = np.exp(admittance_model(f, *popt))
...
if __name__=="__main__":
	L_al = 10 * nH
	filenames = glob.glob(datadir + "Cap_300MHz_500x3440_*umboundary.csv")
	disttoGNDplane = np.array([50, 100, 150, 200, 210, 230, 250, 300, 4183])
	filenames.sort(key=lambda x:int(x.split('/')[-1].split('.')[0].split('_')[-1][:3]))
	mask2 = np.logical_or(disttoGNDplane < 240, disttoGNDplane > 260)
	mask2 = np.ones_like(disttoGNDplane, dtype=bool)
	mask = np.ones_like(disttoGNDplane, dtype=bool)
	#print (filenames)
	Cs = []
	Ls = []
	Rs =  []
	...
	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(disttoGNDplane[mask2], Cs[mask2]/pF, 'bo', ms=12)
	...
	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(disttoGNDplane[mask2], Ls[mask2]/nH, 'bo', ms=12)
