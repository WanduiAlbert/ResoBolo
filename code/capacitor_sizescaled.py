...
if __name__=="__main__":
	...
	h = np.linspace(500, 1100, 100)
	Cfit = np.polyval(pC, h)
	Lfit = np.polyval(pL, h)

	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(expected_size, Cs/pF, 'bo', ms=12)
	ax.plot(h, Cfit, 'k', label=labelC)
	ax.grid(which='both')
	ax.legend(loc='upper left')
	ax.set_xlabel(r'Capacitor Height [um]')
	ax.set_ylabel(r'Capacitance [pF]')
	#ax.set_ylabel(r'Characteristic Impedance [Ohms]')
	plt.savefig(plotdir + 'C_vs_design_size.png')

	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(expected_size, Ls/nH, 'bo', ms=12)
	...
	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(expected_size/MHz, w0s/nH, 'ko')
	ax.grid(which='both')
	...
	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(expected_size/MHz, alphak, 'ko')
	ax.grid(which='both')
	ax.set_xlabel(r'Design Frequency [MHz]')
	...
	fig, ax = plt.subplots(figsize=(10,10))
	ax.plot(expected_size/MHz, epsilons*1e5, 'ko')
	ax.grid(which='both')
	ax.set_xlabel(r'Design Frequency [MHz]')
	...
	p = np.polyfit(expected_size/MHz, freqs/MHz, 1)
	print (p)
	m, b = p
	fig, ax = plt.subplots(figsize=(10,10))
	ax.scatter(expected_size/MHz, freqs/MHz)
	ax.plot(expected_size/MHz, np.polyval(p, expected_size/MHz), 'k',
			label='m=%1.3f\nb=%1.3f'%(m,b))
	ax.grid(which='both')
