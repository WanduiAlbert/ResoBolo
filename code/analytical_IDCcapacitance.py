...
def interior_capacitance(w, g, h, er):
	...
	#k1p = np.sqrt(1-k1**2)
	#Ca = epsilon_0*(er-1)*special.ellipk(k1)/special.ellipk(k1p)
	...
	Cb = epsilon_0*special.ellipk(k1inf)/special.ellipk(k1infp)
	C = epsilon_0*(er+1)*special.ellipk(k1inf)/special.ellipk(k1infp)
...
def exterior_capacitance(w, g, h, er):
	...
	## For the finite layer dielectric
	#t4 = np.cos(pi*(eta+1)/(8*r))
	#t3 = np.cos(pi*(1-eta)/(8*r))
	#kE = 1./t3*np.sqrt((t4**2-t3**2)/(t4**2-1))
	#kEp = np.sqrt(1-kE**2)
	#Ca = epsilon_0*(er-1)*special.ellipk(kE)/special.ellipk(kEp)

	# For the infinite layer dielectric
	kEinf = 2*eta**0.5/(1 + eta)
	kEinfp = np.sqrt(1-kEinf**2)
	Ca = epsilon_0*(er)*special.ellipk(kEinf)/special.ellipk(kEinfp)
	Cb = epsilon_0*special.ellipk(kEinf)/special.ellipk(kEinfp)
	C = epsilon_0*(er+1)*special.ellipk(kEinf)/special.ellipk(kEinfp)
