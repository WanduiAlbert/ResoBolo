...
if __name__=="__main__":
    if not doS21analysis:
        capfilenames = glob.glob(datadir +
                "Cap_*MHz_with_boundary_and_coupcaps.csv")
        capfilenames.sort(key=lambda x: int(x.split("/")[-1].split('.')[0].split('_')[-1][:-3]))
        nsections = list(map(lambda x: int(x.split("/")[-1].split('.')[0].split('_')[-1][:-3]),
            capfilenames))
        indfilename = datadir + "ACTKID_inductor.csv"
        ...
        Nfingers = 502 - np.array([0, 17, 32, 47, 61, 74])#, 87])
        #labels = ['2', '3', '4', '6', '8', '10', '12', '14', '16']
        labels = list(map(lambda x: str(x), nsections))
        #labels = ['4', '6']
        ...
        fns = glob.glob(datadir + "ACTKID_Resonator_*MHz_zoomed.csv")
        fns.sort(key=lambda x: int(x.split("/")[-1].split('.')[0].split('_')[-2][:-3]))
        nsections = list(map(lambda x: int(x.split("/")[-1].split('.')[0].split('_')[-2][:-3]), fns))
        print (nsections)
        frs = np.zeros(len(nsections))
        Qcs = np.zeros(len(nsections))
        phics = np.zeros(len(nsections))
        Nfingers = 502 - np.array([0, 17, 32, 47, 61, 74, 87])
        fr_guess = np.array([323.1, 329.4, 335.3, 341.4, 347.4, 352.3, 359.3 ])
        ...
        plt.figure(123)
        plt.plot(Nfingers[mask], Qcs[mask], 'ko', ls='None', ms=12)
        plt.plot(Nfine, Qcfine, 'r-')
        plt.grid()
        plt.xlabel('Nfingers')
        plt.ylabel('Qc ')
        plt.savefig(plotdir + 'Qc_vs_Nfingers.png')

        p = np.polyfit(Nfingers[mask]/N0, phics[mask], 2)
        print ("phic fitting parameters vs. Nfingers", p)
        phicfine = np.polyval(p, Nfine/N0)
        phicresiduals = (phics - np.polyval(p, Nfingers/N0))/phics
        plt.figure(124)
        plt.plot(Nfingers[mask], phics[mask], 'ko', ls='None', ms=12)
        plt.plot(Nfine, phicfine, 'r-')
        plt.grid()
        plt.xlabel('Nfingers')
        plt.ylabel('phi_c [radians]')
        plt.savefig(plotdir + 'phic_vs_Nfingers.png')

        plt.figure(125)
        plt.plot(Nfingers[mask], Qcresiduals[mask], 'ko', ls='None', ms=12)
        plt.grid()
        plt.xlabel('Nfingers')
        plt.ylabel('fractional Qc residuals')
        plt.savefig(plotdir + 'Qcresiduals_vs_Nfingers.png')
        ...
        plt.savefig(plotdir + 'phicresiduals_vs_Nfingers.png')
        #plt.show()
        plt.close('all')
        ...
        plt.xlabel('fr [MHz]')
        plt.ylabel('fractional Qc residuals')
        plt.savefig(plotdir + 'Qcresiduals_vs_fr.png')

        plt.figure(126)
        plt.plot(frs[mask], phicresiduals[mask], 'ko', ls='None', ms=12)
        plt.grid()
        plt.xlabel('fr [MHz]')
        plt.ylabel('fractional phic residuals')
