...
for k in range(ncf):
    ...
    Qcsinterp = 1./np.real(1./Qesinterp)
    if k == 0 or k == 50 or k == 100:
        pl.figure()
        pl.title("Freq %1.1fMHz"%(f0/1e6))
        pl.plot(xfine, Qcs_pred,'r-', label='analytical')
        pl.plot(xfine, Qcsinterp,'k--')
        pl.plot(fracs, Qcs, 'kh', ls='None', label='from fitting')
        pl.grid()
        pl.legend(loc='upper right')
        pl.ylabel('Qc')
        pl.xlabel('Fractional position along the line')
        #pl.ylim((0, 50000))
        pl.savefig('Qc_variation_with_pos_%1.1fMHz.png'%(f0/1e6))
        #pl.show()
    #continue
    #exit()
    #pl.figure()
    ...
    color = cmap(k/(ncf-1.))
    pl.subplot(211)
    pl.plot(fracs,Qcs,label='%d MHz'%(f0*1e-6),color=color)
    pl.subplot(212)
...
pl.subplot(211)
pl.title('Qc variation with resonator position l=502mm eps=13 Z=30')
pl.grid()
#pl.legend()
pl.ylabel('Qc')
pl.subplot(212)
...
#pl.scatter(frac_position*design_freqs, design_freqs, c='k')
pl.title("|Qe|")
