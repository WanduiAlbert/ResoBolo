...
class OperatingPoint():
	...
	def __init__(self):
		# Material properties
		Tc = 1.5 * u.kelvin
		N0 = 0.87e10 / (u.electron_volt * u.micrometer**3)
		...
		nu = 220 * u.gigahertz
		bw = 0.27
		eta = 0.4					# Front of dewar to dissipation resistor efficiency
		Tsky = 21.* u.kelvin		# Total optical load referenced to front of dewar (rj)
		#Tsky = 43. * u.kelvin
		...
		To = 0.35 * u.kelvin		# Desired operating temperature of bolometer
		Tb = 0.25 * u.kelvin		# Bolometer bath temperature
		beta = 2.0					# Thermal conductivity exponent of legs

		kc = Pt / (To**(beta+1.) - Tb**(beta+1.))

		G = kc * (beta + 1.) * To**beta		# bolometer G at operating temperature
		Gb = kc * (beta+1.) * Tb**beta
		print "Gc: ",G
		...
		C = 16.9*u.picofarad
		fro = (1.0/(2.*pi*np.sqrt(L*C))).to('gigahertz')
		print "fro: ",fro
		...
		#gamma = 0.5
		gammaphonon = ((1 + beta) / (2*beta + 3)) * ((Tb/To)**(3.+2.*beta)-1.)/((Tb/To)**(beta+1.)-1)
