...
niceprint("The operating temperature of the island", T_b)
# T_b = 60 * u.mK
...
f_th = (1/2/np.pi/tau_th).to(u.Hz) # roll off frequency for bolometer
...
P_b = G_b * T_b
tau_b = (C_b/G_b).to('us')
f_b = (1/2/np.pi/tau_b).to(u.Hz) # roll off frequency for bolometer
...
niceprint ("QP Roll off frequency", f_th)
...
#bolo_rolloff =  1/(1 + 1j * 2 * np.pi * nu * tau_b[:, np.newaxis])
#qp_rolloff =  1/(1 + 1j * 2 * np.pi * nu * tau_qp[:, np.newaxis])
#print (bolo_rolloff.shape)
#bolo_rolloff = np.abs(bolo_rolloff)
#qp_rolloff = np.abs(qp_rolloff)
#
#NEP_ph = NEP_ph[np.newaxis, :] * bolo_rolloff
#NEP_amp = NEP_amp[np.newaxis, :]* ones
#NEP_gr = NEP_gr[np.newaxis, :]* qp_rolloff * bolo_rolloff
#NEP_total = (NEP_gr**2 + NEP_amp**2 + NEP_ph**2)**0.5
#
...
# ----------------------------------------------------------------- #
# Let's make a bunch of plots - as a function of P_opt
# 1. P_opt as a function of the temperature of the bolometer
# fig, ax = plt.subplots(figsize=(12,12))
# ax.plot(T_b, P_opt)
# ax.set_xlabel(r'$T_b$ [K]')
# ax.set_ylabel(r'$P_{\texttt{opt}}$ [pW]')
# ax.grid()
# ax.axis('tight')
# plt.savefig('Popt_vs_temp.png')

# plt.show()

# titlestr = r'$P_{\texttt{read}}$' + ' = {0:1.2f}'.format(P_read.to(dBm))

# 2. NEPs vs P_opt

# fig, ax = plt.subplots(figsize=(12,12))
# ax.semilogy(P_opt, NEP_ph, 'b', label='Phonon')
# ax.semilogy(P_opt, NEP_amp, 'r', label='Amplifier')
...
# ax.grid()
# ax.axis('tight')
# plt.savefig('responsivity_vs_temp.png')
# plt.show()
...
# ax.plot(T_b, G_b, 'b')
# ax.set_xlabel(r'$T_b$ [K]')
# ax.set_ylabel(r'$G_b$ [pW/K]')
# ax.grid()
# ax.axis('tight')
# plt.savefig('conductance_vs_temp.png')
# plt.show()

# --------------------------------------------------------------------- #
# Let's make a bunch of plots - as a function of P_read
# 1. P_read as a function of the temperature of the bolometer
# fig, ax = plt.subplots(figsize=(12,12))
# ax.plot(T_b, P_read)
# ax.set_xlabel(r'$T_b$ [K]')
# ax.set_ylabel(r'$P_{read}$ [pW]')
# ax.grid()
# ax.axis('tight')
...
# fig, ax = plt.subplots(figsize=(12,12))
# ax.plot(P_read.to(dBm), r_f, 'b')
# ax.set_xlabel(r'$P_{\texttt{read}}$ [dBm]')
# ax.set_ylabel(r'Responsivity [kHz/pW]')
# ax.grid()
# ax.axis('tight')
# plt.savefig('responsivity_vs_Pread.png')
# plt.show()
