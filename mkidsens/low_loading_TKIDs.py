...
N_pol = 1
P_opt = 1.00*u.pW #((Vdc/Rb)**2 * Rh).to(u.pW)

gamma_leg = 3.131 # conductivity index = beta + 1
K_leg = 403.705 * u.picoWatt/u.Kelvin**gamma_leg
T_c = 1.287 * u.Kelvin
T_0 = 0.043* u.Kelvin # Previously 0.23K Temperature of the thermal bath
...
if __name__=="__main__":
    ...
    frequencies = np.array([258.33, 264.49, 268.70])*u.MHz
    Rb = np.array([510, 510, 510]) * u.kOhm
    Pref = 10e-12
    ...
    #print (P_opt)
    gamma_leg = np.array([2.962, 2.754, 2.862])# conductivity index = beta + 1
    K_leg = np.array([122, 122, 122])* u.picoWatt
