...
r = ((chi_c * chi_qp * beta/4) * tau_qp/tau_th * kappa/P_b).to(1/u.pW) * ones

# S_opt = (2 * (1 + n_opt) * h * nu_opt * P_opt * ones).to(u.aW**2/u.Hz)

S_ph = (4 * k_B * T_b**2 * G_b * ones ).to(u.aW**2/u.Hz)
...
NEP_amp = (S_amp**0.5/r).to(u.aW/u.Hz**0.5) * ones


S_shot = 2 * n_qp * V_sc * (1/tau_max + 1/tau_qp) * ones
...
r_f = (r * (2/(chi_c * chi_g * Q_i) * f_r)).to(u.kHz/u.pW)
