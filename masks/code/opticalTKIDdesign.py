...
if __name__=="__main__":
    ...
    for i in range(N-1):
        ...
        (bxmin, bymin), (bxmax, bymax) = blade_ref.get_bounding_box()
        blade_dx = (bxmax - bxmin)
        dx = all_blades[i] * (trace_width + gap_width) - gap_width/2
        blade_ref.translate(xmin+dx - blade_dx/2, 0)
        bladed_cell.add(blade_ref)
        demo_blades.append(bladed_cell)

    gdspy.write_gds("opticalTKID_IDC.gds",
            cells=[cap_cell, main_blade] + blade_cells + demo_blades,unit=1e-6,
            precision=1e-9)
