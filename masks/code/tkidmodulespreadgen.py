...
def get_cellname(cell):
    if type(cell) == str: return cell
    return cell.name
...
def generate_inverted_cell_in_overlay(cellref, mask_components):
    cellname = get_cellname(cellref.ref_cell)
    #pdb.set_trace()
    print (cellname)
    #if cellname.startswith("cap2bolo_corner"): pdb.set_trace()
    ref_cell = main_lib.cells[cellname]
    invcellname = cellname + "_inv"
    #if cellname == "50umX15mm_Hline_r": pdb.set_trace()
    if invcellname in mask_components:
        invcellref = gdspy.CellReference(main_lib.cells[invcellname])
    elif cellname in mask_components:
        invcellref = gdspy.CellReference(main_lib.cells[cellname])
    else:
        try:
            invcell = main_lib.cells[invcellname]
            invcellref = gdspy.CellReference(invcell)
        except KeyError:
            invcell = gdspy.Cell(invcellname)
            if ref_cell.get_dependencies():
                subcells = ref_cell.references
            else:
                msg = patches.bcolors.WARNING
                msg += "%s: NO Matching cell or inv cell in any reticle"%cellname
                msg += patches.bcolors.ENDC
...
def generate_inverted_overlay(wafer, mask_components):
    ...
    invwafer = gdspy.Cell('Wafer_Layout_new_inverted')
    gcomponents = wafer.references
    #pdb.set_trace()
...
if __name__ == "__main__":
    ...
    if makeInvertedOverlay:
        main_lib.read_gds(final_fn)
        cells = main_lib.cells
        cell_dict = list(cells.keys())

        for cell in cell_dict:
            if cell.startswith('Wafer_Layout_new_inverted'): main_lib.remove(cell)
        ## Give the name of the top cell in the file just read.
        top = cells['Wafer_Layout_new']
        masklist = cells['TKIDModule_Main_Reticle'].get_dependencies()
        masklist.update(cells['TKIDModule_Capacitor_Reticle'].get_dependencies())
    ...
    # spreadsheet.
    layer_order = [3, 12, 9, 6, 13, 4, 5, 2, 8, 1, 11, 10, 14, 23]
