...
class IDC():
	...
	def draw(self, less_one=False):
		...
		self.left_contact.layers[0] = self.layer
		self.left_contact.translate(-dx, -dy/2)
		self.right_contact.translate(-dx, -dy/2)
		self.right_contact.layers[0] = self.layer
		self.cell = gdspy.Cell(self.cellname)
		for f in self.fingers:
			f.layers[0] = self.layer
			f.translate(-dx, -dy/2)
			self.cell.add(f)
...
def get_alignment_marks():
	fn = '../mask_files/alignment_marks.gds'
...
def get_inductor():
	fn = '../mask_files/new_inductor.gds'
	gdsii = gdspy.GdsLibrary()
	gdsii.read_gds(fn)
...
def get_island():
	fn = '../mask_files/LSN_Island_280umlegs-R.gds'
	gdsii = gdspy.GdsLibrary()
	gdsii.read_gds(fn)
...
def get_XeF2_release():
	fn = '../mask_files/XeF2_release.gds'
	gdsii = gdspy.GdsLibrary()
	gdsii.read_gds(fn)
...
def generate_globaloverlay(nrows, ncols):
	...
	for index in unq_indices:
		...
		print (coupcaps[index].C/pF)
	for ires in range(N_res):
		coupcaps[ires].nfinger = coup_Nfingers[ires]
		coupcaps[ires].capacitance()
	...
	for ires, coup_index in zip(range(N_res), unq_inv):
		...
		obtained_Cs.append(idcs[ires].capacitance())
		obtained_Ccs.append(coupcaps[ires].capacitance())

	obtained_Cs = np.array(obtained_Cs)
	obtained_Ccs = np.array(obtained_Ccs)
	print (obtained_Cs/pF)
	print (obtained_Ccs/pF)
	fig, ax = plt.subplots(figsize=(10, 10))
	ax.plot(fs, obtained_Ccs/pF, 'b', label='realized')
	ax.plot(fs, Ccs/pF, 'r', label='expected')
	ax.legend(loc='best')
	ax.set_xlabel(r'Frequency [MHz]')
	ax.set_ylabel(r'Cc [pF]')
	ax.grid(which='both')
	plt.show()
	exit()
...
def make_inverted_cell(cellref, mask_components):
	...
	if cellname == "LSN_Island_280umlegs": pdb.set_trace()
